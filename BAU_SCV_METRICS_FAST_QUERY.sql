/* FAST RUN QUERY FOR DATA LOADING */
...
(sel calendar_date as event_actual_dt, day_of_week from
 ...
 left join
(sel event_actual_dt,  count(*) as RMGTT from edw_scver_coda_accs_views.v_event_party
...
on c1.event_actual_dt = x1.event_actual_dt
 left JOIN
(sel event_actual_dt,  count(*) as TODS from edw_scver_coda_accs_views.v_event_party
where event_actual_dt between (current_date - 30) and current_date 
and data_source_type_id = 2
group by 1) x2
ON c1.event_actual_dt = x2.event_actual_dt
left JOIN  
(sel event_actual_dt, count(*) as POISE from edw_scver_coda_accs_views.v_event_party
where event_actual_dt between (current_date - 30) and current_date 
and data_source_type_id = 5
group by 1) x5
 ON c1.event_actual_dt = x5.event_actual_dt
left JOIN  
(sel event_actual_dt, count(*) as MEARS from edw_scver_coda_accs_views.v_event_party
where event_actual_dt between (current_date - 30) and current_date 
and data_source_type_id = 8
group by 1) x8
 ON c1.event_actual_dt = x8.event_actual_dt
left JOIN  
(sel event_actual_dt,  count(*) as PARCELS from edw_scver_coda_accs_views.v_event_party
where event_actual_dt between (current_date - 30) and current_date 
and data_source_type_id = 11
group by 1) x11
 ON c1.event_actual_dt = x11.event_actual_dt
left JOIN  
(sel transactionDate as event_actual_dt ,  count (*) as MARS
...
left JOIN  
(sel recorded_appl_dt as event_actual_dt,  count (*) as CREDO
...
left JOIN  
(sel cast(tran_start_dttm as date format 'yyyy-mm-dd') as event_actual_dt , count (*) as KOGNITIO
 from
...
left JOIN  
(sel open_dt as event_actual_dt, count (*) as SIEBEL
 from
...
left JOIN  
(sel event_actual_dt,  count(*) as SPS
 from
